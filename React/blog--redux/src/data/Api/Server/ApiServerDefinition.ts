...
function createPostData ({
  user
}: PostDataOptions): ApiServerPostTypeEntity {
  return createApiServerPostTypeEntity({
...
function createUserData (): ApiServerUserTypeEntity {
  const firstName = faker.person.firstName()
  const lastName = faker.person.lastName()
  const name = `${firstName} ${lastName}`;
  const username = faker.internet.userName();

  return createApiServerUserTypeEntity({
    firstName,
    lastName,
    name,
    username,
...
function convertToPostDomainEntity (dbEntity: ApiServerPostTypeEntity): PostDomainEntity {
  const {
    id,
    title,
    date,
...
function generateRandomNotifications (
  ...
  const result = [...Array(numNotifications)].map(() => {
    ...
    return createApiServerNotificationTypeEntity({
      id: nanoid(),
