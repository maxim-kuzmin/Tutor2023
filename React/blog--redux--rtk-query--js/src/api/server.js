...
function getRandomInt(min, max) {
  ...
  return Math.floor(rng() * (realMax - realMin + 1)) + realMin;
}
...
for (let i = 0; i < NUM_USERS; i += 1) {
  ...
  for (let j = 0; j < POSTS_PER_USER; j += 1) {
...
export const handlers = [
  ...
  rest.get('/fakeApi/users', (req, res, ctx) => res(ctx.delay(ARTIFICIAL_DELAY_MS), ctx.json(db.user.getAll()))),
];
...
function generateRandomNotifications(since, numNotifications) {
  const now = new Date();
  let pastDate;
